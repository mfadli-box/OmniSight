...
func (r *repository) GetPrivilege(userID, companyID, moduleID string) (string, error) {
	query := `
	...
	`
	var level string
	err := r.db.QueryRowContext(context.Background(), query, userID, companyID, moduleID).Scan(&level)
	if err != nil {
		return "", err
	}
	return level, nil
