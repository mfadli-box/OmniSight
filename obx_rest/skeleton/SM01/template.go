...
type UserCompanySelectItem struct {
	ID          string `json:"id"`
	CompanyID   string `json:"company_id"`
	CompanyName string `json:"company_name"`
	IsActive    bool   `json:"is_active"`
...
type UserAreaCreateRequest struct {
	AreaID   string `json:"area_id" binding:"required"`
	IsActive *bool  `json:"is_active"`
}

type UserAreaUpdateRequest struct {
	IsActive bool `json:"is_active"`
}

type AreaSelectItem struct {
	ID   string `json:"id"`
	Code string `json:"code"`
	Name string `json:"name"`
}
...
type Repository interface {
	...
	ListUserCompanySelect(userID string) ([]UserCompanySelectItem, error)
	...
	CountUserArea(userID, search string) (int, error)
	ListUserArea(userID, search string, page, size int, sortBy, sortOrder string) ([]UserAreaListItem, error)
	CreateUserArea(userID string, req UserAreaCreateRequest) error
	UpdateUserArea(id string, req UserAreaUpdateRequest) error
	DeleteUserArea(id string) error
	ListAreaByCompany(companyID string) ([]AreaSelectItem, error)
}
...
type UseCase interface {
	...
	UpdateUserPrivilege(id string, req UserPrivilegeUpdateRequest) error
	DeleteUserPrivilege(id string) error
	ListUserArea(userID string, meta mechanic.ActionMeta) ([]UserAreaListItem, mechanic.GridMeta, error)
	CreateUserArea(userID string, req UserAreaCreateRequest) error
	UpdateUserArea(id string, req UserAreaUpdateRequest) error
	DeleteUserArea(id string) error
	ListAreaByCompany(companyID string) ([]AreaSelectItem, error)
