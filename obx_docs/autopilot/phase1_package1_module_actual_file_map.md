# Peta File Nyata per Modul - Paket 1 Fase 1

Dokumen ini memecah Paket 1 Fase 1 menjadi peta file nyata per modul inti agar implementasi bisa dilakukan satu modul per batch.

## Tujuan

1. Menjadikan Paket 1 siap dieksekusi per modul.
2. Menyediakan daftar file nyata untuk backend, frontend, dan dokumentasi.
...
1. Kerjakan satu modul inti dalam satu batch kecil.
2. Update backend, frontend, dan dokumentasi modul tersebut bersama-sama.
3. Verifikasi build dan lint setelah batch selesai.
4. Lanjutkan ke modul inti berikutnya.

## Risiko

1. Terlalu banyak modul inti dibuka sekaligus.
2. Route, page, dan dokumentasi tidak sinkron antar modul.
3. Perubahan kecil tidak diverifikasi sehingga regresi lolos.

## Mitigasi

1. Fokus satu modul per batch.
2. Gunakan map ini sebagai daftar file kerja saat coding.
3. Jalankan verifikasi setelah setiap batch.
