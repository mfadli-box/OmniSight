...
export default function Page() {
  ...
  useEffect(() => {
    ...
    if (!session) return;
    clientApi<{ data: ModuleListItem[] }>("/SM02/select")
      .then((res) => {
  ...
  const handleCreate = async (values: CreateForm) => {
    setSmLoading(true);
    const session = parseSession(window.localStorage.getItem(storageKey));
    if (!session) { setSmLoading(false); return; }
    ...
    try {
      ...
      if (err instanceof ClientApiError && err.status === 401) { setSmLoading(false); return; }
  ...
  const handleUpdate = async (values: UpdateForm) => {
    setSmLoading(true);
    if (!selectedRow) { setSmLoading(false); return; }
    const session = parseSession(window.localStorage.getItem(storageKey));
    if (!session) { setSmLoading(false); return; }
    const hasParent = !!values.parent_id;
    try {
                  ...
                  render={({ field }) => (
                    <Field className="gap-1.5 pt-2 px-2">
                      ...
                      <SearchSelect
                        ...
                        onValueChange={(v) => {
                          field.onChange(v);
                          if (!v) createForm.setValue("is_page", false);
                        }}
